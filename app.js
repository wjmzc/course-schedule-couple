...
function dblChip(kind, who, c){
  ...
  return '<span class="chip chip-'+kind+'"><span class="chip-name">'+who+'·'+esc(c.name)+'</span>'+loc+'</span>';
}
function renderDoubleView(){
  ...
  for(const b of bands){
    const [st,en]=b;
    html += '<tr><td class="time-col">第'+st+(en>st? '-'+en:'')+'节</td>';
    for(let d=1; d<=7; d++){
  ...
  html += '</tbody></table></div>';
  html += '<p class="mode-note">蓝=我的课 ｜ 粉=对象课 ｜ 📍 为上课教室 ｜ 绿格=两人都空。点课名改课请切回 👤 单人。</p>';
  tt.innerHTML = html;
}
