...
function renderDoubleView(){
  ...
  for(const b of bands){
    ...
    for(let d=1; d<=7; d++){
      ...
      if(free){ html += '<span class="free-hint">✓ 都空</span>'; }
        ...
        if(mcs.length) html += mcs.slice(0,2).map(c=>'<span class="chip chip-me">我·'+esc(c.name)+'</span>').join('');
        if(pcs.length) html += pcs.slice(0,2).map(c=>'<span class="chip chip-p">对象·'+esc(c.name)+'</span>').join('');
      }
      html += '</td>';
    }
    html += '</tr>';
  ...
  html += '</tbody></table></div>';
  html += '<p class="mode-note">蓝=我的课 ｜ 粉=对象课 ｜ 绿格=两人都空。点课名改课请切回 👤 单人。</p>';
  tt.innerHTML = html;
}
