...
const EDITION = (document.body && document.body.dataset && document.body.dataset.edition) || 'couple';
function esc(s){ return String(s==null?'':s).replace(/[&<>"']/g, c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])); }

function defaultPeriods(){
  // 银川科技学院教学安排表（2024—2025—1 学期起执行）：A1-A6 教学楼的时间
  const times = ['08:00-08:45','08:55-09:40','10:00-10:45','10:55-11:40','14:00-14:45','14:50-15:35',
    '15:55-16:40','16:45-17:30','19:00-19:45','19:50-20:35','20:45-21:30','21:40-22:25'];
  return times.map((t,i)=>({ label:String(i+1), time:t }));
}
function defaultPeriodsAlt(){
  // 同一节课在 A7/A8/A9 教学楼、T楼、操场的时间不一样（上午 3、4 节错峰），留空=与上面相同
  const times = ['','','10:10-10:55','11:05-11:50','','','','','','','',''];
  return times.map((t,i)=>({ label:String(i+1), time:t }));
}
// A1-A6、D楼 用上面这套时间；A7/A8/A9、T楼、G 楼 用“差异时间”（上午 3、4 节错峰）
const DEFAULT_ALT_BUILDINGS = 'A7,A8,A9,T楼,G,操场';

/* 教学楼归属：同一节次在不同楼栋，上下课时间可能不同 */
function buildingKeyOf(loc){
  const s = String(loc==null? '' : loc).trim();
  if(!s) return '';
  let m = s.match(/^([A-Za-z]{1,3})\s*-?\s*(\d{1,2})/);
  if(m) return (m[1]+m[2]).toUpperCase();          // A8-502 → A8 ／ G1-402 → G1
  m = s.match(/^([A-Za-z])\s*楼/);
  if(m) return m[1].toUpperCase()+'楼';            // T楼
  if(/操场|运动场|体育场/.test(s)) return '操场';
  return '';
}
function altBuildingList(){
  return String(state.altBuildings||'').split(/[,，、\s]+/).filter(Boolean);
}
function usesAltTime(course){
  const k = buildingKeyOf(course && course.location);
  if(!k) return false;
  const key = k.toUpperCase();
  // 支持只写楼号字母：写 “G” 也能匹配 G1-402
  return altBuildingList().some(b=>{
    const x = b.toUpperCase();
    return key===x || key.startsWith(x);
  });
}
function periodTimeAt(idx, useAlt){
  const base = state.periods[idx-1];
  if(!base) return '';
  if(useAlt){
    const alt = (state.periodsAlt||[])[idx-1];
    if(alt && alt.time) return alt.time;
  }
  return base.time || '';
}
function splitTimeRange(t){
  const parts = String(t||'').split(/[-—–~～]/);
  return { start:(parts[0]||'').trim(), end:(parts.slice(1).join('-')||'').trim() };
}
// 一门课实际的上下课时间（按它所在的教学楼取）
function courseTimeRange(c){
  if(!c) return '';
  const useAlt = usesAltTime(c);
  const s = splitTimeRange(periodTimeAt(c.start, useAlt)).start;
  const e = splitTimeRange(periodTimeAt(c.end, useAlt)).end;
  return (s && e) ? (s+'-'+e) : (s || e || '');
}
// 该节次在“部分教学楼”的差异时间（没有差异时返回空）
function altPeriodTime(idx){
  const t = ((state.periodsAlt||[])[idx-1]||{}).time || '';
  const base = (state.periods[idx-1]||{}).time || '';
  return (t && t!==base) ? t : '';
}
function altShortLabel(){
  const list = altBuildingList();
  if(!list.length) return '其他楼';
...
let state = {
  ...
  periodsAlt: defaultPeriodsAlt(),
  altBuildings: DEFAULT_ALT_BUILDINGS,
  mappins: [],
...
function load(){
  try{
    ...
    state.periodsAlt = (Array.isArray(d.periodsAlt) && d.periodsAlt.length) ? d.periodsAlt : defaultPeriodsAlt();
    state.altBuildings = (typeof d.altBuildings==='string' && d.altBuildings.trim()) ? d.altBuildings : DEFAULT_ALT_BUILDINGS;
    ...
    state.uiMode = (d.uiMode==='double') ? 'double' : 'single';
    state.courses2 = (Array.isArray(d.courses2)? d.courses2 : []).map(c=>({
...
function renderTimetable(){
  ...
  updateViewSwitch();
  updateModeSwitch();
  if(state.uiMode==='double'){ renderDoubleView(); return; }
  ...
  for(let i=0;i<total;i++){
    ...
    const ap = altPeriodTime(i+1);
    html += '<div class="rowsep"><b>' + esc(p.label) + '</b>' + (p.time? '<span>'+esc(p.time)+'</span>':'')
      + (ap? '<span class="tt-alt" title="'+esc(altBuildingList().join('、'))+'：'+esc(ap)+'">'+esc(altShortLabel())+' '+esc(ap)+'</span>' : '')
      + '</div>';
  }
  html += '</div>';
  ...
  for(let d=1; d<=7; d++){
    ...
    for(const lane of lanes){
      ...
      html += '<div class="course-card" data-id="'+esc(c.id)+'" style="top:'+top+'px;height:'+height+'px;left:'+left+';width:'+width+';background:'+pal.bg+';color:'+pal.fg+';border-left-color:'+pal.edge+'">'
        ...
        + (ctime? '<div class="c-meta">🕘 '+esc(ctime)+'</div>':'')
...
/* ---------- 设置对话框 ---------- */
// 解析“节次|开始-结束”文本；按节次编号定位，跳过空行的节次不会被填错位置
function parsePeriodLines(text){
  const out = [];
  String(text||'').split(/\r?\n/).forEach((ln)=>{
    const t = ln.trim(); if(!t) return;
    const [rawLabel, rawTime] = t.split('|');
    const label = (rawLabel||'').trim() || String(out.length+1);
    const idx = /^\d+$/.test(label) ? (parseInt(label,10)-1) : out.length;
    while(out.length < idx) out.push({ label:String(out.length+1), time:'' });
    out[idx] = { label, time: (rawTime||'').trim() };
  });
  return out;
}
function openSettings(){
  ...
  $('s_periodsAlt').value = (state.periodsAlt||[]).map(p=> (p.label||'') + (p.time? '|'+p.time : '')).join('\n');
  $('s_altBuildings').value = state.altBuildings || DEFAULT_ALT_BUILDINGS;
  $('settingsDlg').showModal();
}
function saveSettings(){
  const total = clampInt($('s_totalWeeks').value, 1, 40, 20);
  const startDate = $('s_startDate').value || '';
  // 按“节次”编号定位，避免删掉某一行后整体错位
  const periods = parsePeriodLines($('s_periods').value);
  const periodsAlt = parsePeriodLines($('s_periodsAlt').value);
  const altBuildings = $('s_altBuildings').value.trim();
  state.totalWeeks = total;
  state.startDate = startDate;
  if(periods.length) state.periods = periods;
  state.periodsAlt = periodsAlt;
  state.altBuildings = altBuildings || DEFAULT_ALT_BUILDINGS;
...
function importBackupFile(file){
  readFileText(file).then(text=>{
    try{
      ...
      let slot = 1;
      if(EDITION!=='public'){
        const which = window.prompt('这份备份恢复到哪个课表？\n输入 1 = 我的课表\n输入 2 = 对象课表', '1');
        if(which===null) return;
        slot = String(which).trim()==='2' ? 2 : 1;
      }
      if(!confirm('确定恢复吗？将覆盖【'+(slot===1?'我的':'对象')+'】当前的课表。')) return;
      const list = (s.courses||[]).map(normCourse);
      if(slot===1) state.courses = list; else state.courses2 = list;
      state.totalWeeks = clampInt(s.totalWeeks, 1, 40, state.totalWeeks);
      if(Array.isArray(s.periods) && s.periods.length) state.periods = s.periods;
      if(Array.isArray(s.periodsAlt) && s.periodsAlt.length) state.periodsAlt = s.periodsAlt;
      if(typeof s.altBuildings==='string' && s.altBuildings.trim()) state.altBuildings = s.altBuildings;
      if(s.startDate) state.startDate = s.startDate;
      save(); renderAll();
      toast('已恢复到【'+(slot===1?'我的':'对象')+'】课表（'+list.length+' 门课）');
    }catch(e){
...
/* ---------- 启动 ---------- */
load();
  if(EDITION==='public'){ state.uiMode='single'; const ms=$('modeSwitch'); if(ms) ms.style.display='none'; }
bindEvents();
...
function updateModeSwitch(){
  document.querySelectorAll('#modeSwitch .vs-btn').forEach(b=>b.classList.toggle('active', b.dataset.mode===state.uiMode));
}
function normCourse(c){
  return { id: c.id||uid(), name: c.name||'未命名', teacher: c.teacher||'', location: c.location||'',
    day: clampInt(c.day,1,7,1), start: clampInt(c.start,1,30,1), end: clampInt(c.end, c.start||1,30,c.start||1),
    weeksRaw: c.weeksRaw||'', color: c.color!=null? c.color : colorIndex(c.name) };
}
function bandsOf(){
  const map = {};
  for(const c of state.courses.concat(state.courses2)){
    if(c.start>=1 && c.end>=c.start) map[c.start+'-'+c.end] = [c.start, c.end];
  }
  return Object.values(map).sort((a,b)=>a[0]-b[0]);
}
// 双人视图里的一个课程块：课程名 + 教室位置
function dblChip(kind, who, c){
  const loc = c.location ? '<span class="chip-loc">📍 '+esc(c.location)+'</span>' : '';
  const tm = courseTimeRange(c);
  const t = tm ? '<span class="chip-time">🕘 '+esc(tm)+'</span>' : '';
  return '<span class="chip chip-'+kind+'"><span class="chip-name">'+who+'·'+esc(c.name)+'</span>'+loc+t+'</span>';
}
function renderDoubleView(){
  const tt = $('timetable');
  const week = state.currentWeek;
  const pCount = state.courses2? state.courses2.length : 0;
  if(!state.courses || !state.courses.length || !pCount){
    tt.innerHTML = '<div class="empty-note">双人视图需要两份课表：<br>① 先恢复【我的课表】；<br>② 再点 ⋮ 更多 → 📂 从备份恢复，输入 2，把对象备份恢复为【对象课表】。</div>';
    return;
  }
  const bands = bandsOf();
  const mine = state.courses, part = state.courses2;
  const act = (c)=> courseActiveWeeks(c).weeks.includes(week);
  const overlap = (c, st, en)=> c.day>=1 && c.day<=7 && c.start<=en && c.end>=st && act(c);
  // 每个节次上是否有人上课（用于计算都空时段）
  const maxN = Math.max(1, ...bands.map(b=>b[1]), ...mine.map(c=>c.end), ...part.map(c=>c.end));
  const busySet = (courses, d, slot)=> courses.some(c=> c.day===d && slot>=c.start && slot<=c.end && act(c));
  const freeRuns=[];
  for(let d=1; d<=7; d++){
    let run=null;
    for(let s=1; s<=maxN; s++){
      const free = !busySet(mine,d,s) && !busySet(part,d,s);
      if(free){ if(!run) run=[s,s]; else run[1]=s; }
      else if(run){ freeRuns.push({d, st:run[0], en:run[1]}); run=null; }
    }
    if(run) freeRuns.push({d, st:run[0], en:run[1]});
  }
  let html = '<div class="dbl-summary"><b>第 '+week+' 周 · 两人都空的时段：</b>' +
    (freeRuns.length? freeRuns.map(r=>DAY_NAMES[r.d-1]+' 第'+r.st+(r.en>r.st? '-'+r.en:'')+'节').join('　；　') : '本周暂无（可切到别的周看看）') + '</div>';
  html += '<div class="dbl-wrap"><table class="dbl-tbl"><thead><tr><th class="time-col">节次</th>';
  for(let d=1; d<=7; d++) html += '<th>'+DAY_NAMES[d-1]+'</th>';
  html += '</tr></thead><tbody>';
  for(const b of bands){
    const [st,en]=b;
    const bStart = splitTimeRange(periodTimeAt(st,false)).start;
    const bEnd = splitTimeRange(periodTimeAt(en,false)).end;
    const bAltS = altPeriodTime(st), bAltE = altPeriodTime(en);
    let bAlt = '';
    if(bAltS || bAltE){
      const as = splitTimeRange(bAltS||periodTimeAt(st,false)).start;
      const ae = splitTimeRange(bAltE||periodTimeAt(en,false)).end;
      bAlt = '<span class="row-alt" title="'+esc(altBuildingList().join('、'))+'">'+esc(altShortLabel())+' '+esc(as+'-'+ae)+'</span>';
    }
    html += '<tr><td class="time-col">第'+st+(en>st? '-'+en:'')+'节'
      + ((bStart&&bEnd)? '<span class="row-time">'+esc(bStart+'-'+bEnd)+'</span>' : '')
      + bAlt + '</td>';
    for(let d=1; d<=7; d++){
      const mcs = mine.filter(c=>c.day===d && overlap(c,st,en));
      const pcs = part.filter(c=>c.day===d && overlap(c,st,en));
      const free = !mcs.length && !pcs.length;
      html += '<td'+(free? ' class="bothfree"' : '')+'>';
      if(free){ html += '<span class="free-hint">✓ 都空</span>'; }
      else {
        if(mcs.length) html += mcs.slice(0,2).map(c=>dblChip('me','我',c)).join('');
        if(pcs.length) html += pcs.slice(0,2).map(c=>dblChip('p','对象',c)).join('');
        const rest = Math.max(0, mcs.length-2) + Math.max(0, pcs.length-2);
        if(rest) html += '<span class="chip-more">还有 '+rest+' 门…</span>';
      }
      html += '</td>';
    }
    html += '</tr>';
  ...
  html += '</tbody></table></div>';
  html += '<p class="mode-note">蓝=我的课 ｜ 粉=对象课 ｜ 📍 上课教室 ｜ 🕘 上下课时间（按教学楼错峰）｜ 绿格=两人都空。点课名改课请切回 👤 单人。</p>';
  tt.innerHTML = html;
}
function bindModeEvents(){
  document.querySelectorAll('#modeSwitch .vs-btn').forEach(b=>{
    b.addEventListener('click', ()=>{ state.uiMode = b.dataset.mode; save(); renderAll(); });
  });
}
...
function renderListView(){
  ...
  for(let d=1; d<=7; d++){
    ...
    for(const c of cs){
      ...
      const aw = courseActiveWeeks(c);
      const tag = (aw.text && aw.text!=='全周') ? aw.text : '';
      const tr = courseTimeRange(c) || periodRangeText(c.start, c.end);
